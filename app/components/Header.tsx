...
export function Header() {
  ...
  return (
    <>
        ...
        <div className="container-site flex h-24 items-center justify-between">
          ...
          <div className="flex items-center gap-4 lg:hidden">
            <a
              ...
              className="flex size-11 items-center justify-center rounded-full bg-[var(--pink)] text-white shadow-[0_12px_30px_rgba(217,140,160,0.34)]"
            ...
            >
              <span className={`block h-px w-8 bg-black transition ${isOpen ? "translate-y-[9px] rotate-45" : ""}`} />
              <span className={`block h-px w-8 bg-black transition ${isOpen ? "opacity-0" : ""}`} />
              <span className={`block h-px w-8 bg-black transition ${isOpen ? "-translate-y-[9px] -rotate-45" : ""}`} />
            </button>
          </div>
        </div>
      ...
      <div
        className={`fixed inset-0 z-[60] bg-[var(--background)] transition-all duration-500 lg:hidden ${
          isOpen
            ? "pointer-events-auto translate-y-0 opacity-100"
            : "pointer-events-none -translate-y-4 opacity-0"
        }`}
      >
        <div className="container-site flex min-h-dvh flex-col pt-32 pb-8">
          <nav className="flex flex-col">
            {menuItems.map((item) => (
              <a
                ...
                href={item.href}
                onClick={closeMenu}
                className="border-b border-[var(--border)] py-5 text-[30px] font-normal leading-none tracking-[-0.03em] text-[var(--foreground)]"
              >
                {item.label}
              </a>
            ))}
